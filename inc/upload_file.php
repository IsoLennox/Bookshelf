...
// if everything is ok, try to upload file
 ...
 $uploaded_by = $_SESSION["user_id"];
    ...
    $authorresult1 = mysqli_query($connection, $authorquery1);
      ...
      foreach($authorresult1 as $this_author){
...
//            $author_id=$author_array['id'];
            ...
            $update_book .= "author_id = '{$this_author['id']}' ";
            $update_book .= "WHERE id = {$book_id} "; 
            $result = mysqli_query($connection, $update_book);

            if ($result && mysqli_affected_rows($connection) == 1) {
              // Success
               $_SESSION["message"] = "File Upload Successful! Add a genre and a description!";          
              redirect_to("edit.php?book_id=$book_id");

            } else {
              // Failure
              $_SESSION["message"] = "Author: ".$author." #".$author_id." not added to book";
