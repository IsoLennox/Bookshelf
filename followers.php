...
echo "<h4><a href=\"following.php?user=".$user_id."\">".$num_following." Following</a> | ".$num_followers." Followers <span class=\"right\"><a href=\"all_users.php\" title=\"Explore Users\"><i class=\"fa fa-globe\"></i></a></span> </h4> ";



//GET USERS FOOLOWING THIS USER

    $query  = "SELECT * FROM following WHERE following={$user_id}";  
    $result = mysqli_query($connection, $query);
    if($result){
        
         $following_you=mysqli_fetch_assoc($result);
        if(empty($following_you)){
    ...
    foreach($result as $user){
        ...
        $get_user  = "SELECT * FROM users WHERE id={$user['you']}";  
